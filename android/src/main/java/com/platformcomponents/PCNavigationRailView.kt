 ...
 * whether it was already selected. The destinations are built like TabBar's
 * tabs (PCNavigationBarSupport).
 *
...
class PCNavigationRailView(context: Context) :
  ...
  fun applyItems(value: List<Item>) {
    ...
    val r = rail
    if (badgesOnly && r != null) {
      PCNavigationBarSupport.applyBadges(r, items, badgeBackgroundColor, badgeTextColor)
    } else {
      rebuildUI()
    }
  }
  ...
  fun applyBadgeColors(background: Int?, text: Int?) {
    ...
    badgeTextColor = text
    rail?.let { PCNavigationBarSupport.applyBadges(it, items, badgeBackgroundColor, badgeTextColor) }
  }
  ...
  private fun rebuildUI() {
    ...
    PCNavigationBarSupport.applyBadges(r, items, badgeBackgroundColor, badgeTextColor)
    updateSelection()
    PCNavigationBarSupport.applyTestIDsAndFonts(r, items, labelFont)
    requestLayout()
    reportIntrinsicSize()
  }
  ...
  private fun updateSelection() {
    ...
    if (index in items.indices) {
      ...
      if (r.selectedItemId != PCNavigationBarSupport.itemId(index)) r.selectedItemId = PCNavigationBarSupport.itemId(index)
    } else {
