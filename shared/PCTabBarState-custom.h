...
/**
 * Custom state for TabBar that holds the measured frame size from native.
 * This allows the native side to measure the actual tab bar and communicate
 * the size to the shadow node for proper Yoga layout.
 */
struct PCTabBarStateFrameSize {
  using Shared = std::shared_ptr<const PCTabBarStateFrameSize>;

  Size frameSize{}; // {width, height} in points

  PCTabBarStateFrameSize() = default;

  explicit PCTabBarStateFrameSize(Size size) : frameSize(size) {}

  bool operator==(const PCTabBarStateFrameSize& other) const {
  ...
  PCTabBarStateFrameSize(
      const PCTabBarStateFrameSize& previousState,
      folly::dynamic data)
