...
/**
 * A destination: the same item as a TabBar tab. `role: 'search'` gives a
 * destination without an icon the search icon; `systemItem` is TabBar only.
 */
...
export function NavigationRail(props: NavigationRailProps): React.ReactElement {
  ...
  // The expanded rail shows every destination
  const nativeItems = useMemo(
    () => toNativeTabItems(items, items.length),
    [items]
  );
