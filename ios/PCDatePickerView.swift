  ...
  UIAdaptivePresentationControllerDelegate
{
  ...
  public var mode: String = "date" {
    didSet { if oldValue != mode { applyMode(); invalidateSize() } }
  }

  /// "modal" | "inline"
  public var presentation: String = "modal" {
    didSet { if oldValue != presentation { applyPresentation(); invalidateSize() } }
  }
  ...
  public var preferredStyle: String? {
    didSet { if oldValue != preferredStyle { applyPreferredStyle(); invalidateSize() } }
  }
  ...
  @objc public func sizeForLayout(withConstrainedTo constrainedSize: CGSize) -> CGSize {
    guard presentation == "inline" else { return .zero }

    picker.setNeedsLayout()
    picker.layoutIfNeeded()

    let width = (constrainedSize.width.isFinite && constrainedSize.width > 1) ? constrainedSize.width : 320
  ...
  private func applyOpen() {
    ...
    if shouldOpen { presentIfNeeded() }
    else { dismissIfNeeded(emitCancel: false) }
  }
  ...
  private func applyMinMax() {
    ...
    if let ms = minDateMs?.doubleValue { picker.minimumDate = Date(timeIntervalSince1970: ms / 1000.0) }
    else { picker.minimumDate = nil }

    if let ms = maxDateMs?.doubleValue { picker.maximumDate = Date(timeIntervalSince1970: ms / 1000.0) }
    else { picker.maximumDate = nil }
  }
